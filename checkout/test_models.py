...
class TestCheckoutModel(TestCase):
    ...
    def test_update_total(self):
        self.assertEqual(
            self.order.grand_total, self.item1.price + self.combo.price +
    ...
    def test_changeform_link(self):
        # Assess the change link in admin page for the Order returns correct
        # url
        self.assertEqual(
            self.oli2.changeform_link(),
            '<a href="/admin/checkout/orderlineitem/2/change/" target="_blank">\
See Contents</a>'
            )
    ...
    def test_orderlineitem_save(self):
        ...
        self.assertNotEqual(self.oli.lineitem_total,
                            self.oli.quantity * self.oli.food_item.price)

        self.oli.save()
        self.assertEqual(
            self.oli.lineitem_total,  self.oli.quantity *
            self.oli.food_item.price)

    def test_orderlineitem_string_method(self):
        # Check individual item in order line item is stringed correctly
        self.assertEqual(str(self.oli), 'Item Id 1 on order ' +
                         self.order.order_number)

        # Check Combo item in order line item is stringed correctly
        self.assertEqual(str(self.oli2), 'Combo Id 1 on order ' +
                         self.order.order_number)
    ...
    def test_combolineitem_string_method(self):
        ...
        self.assertEqual(
            str(self.cli), 'Food item Id: '+str(self.item1.pk) +
            ' added to combo '+self.oli2.combo_id)
        self.assertEqual(str(self.cli2), 'Food item Id: ' + str(self.item2.pk)
                         + ' added to combo '+self.oli2.combo_id)
