...
class TestFoodOrderView(TestCase):
    ...
    def test_recalculate_remaining_delivery_amount(self):
        ...
        data = {'total': 20.00}
        response = self.c.get('/food_order/recalculate_remaining_delivery_amount/', data)
        # IF total < threshold: remaining delivery amount is the difference between them, otherwise 0
        self.assertJSONEqual(str(response.content,encoding='utf8'), {'remaining_delivery_amount': 0.00})
        self.assertEqual(response.status_code, 200)
        ...
        data = {'total': 10.00}
        response = self.c.get('/food_order/recalculate_remaining_delivery_amount/', data)
        self.assertJSONEqual(str(response.content, encoding='utf8'), {'remaining_delivery_amount': 5.00})
        self.assertEqual(response.status_code, 200)